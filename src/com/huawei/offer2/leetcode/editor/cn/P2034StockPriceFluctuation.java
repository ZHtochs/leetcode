...
public class P2034StockPriceFluctuation {
    ...
    class StockPrice {
        ...
        public StockPrice() {
            hash = new HashMap<>();
            minQueue = new PriorityQueue<>(Comparator.comparingInt(ints -> ints[1]));
            maxQueue = new PriorityQueue<>((ints, t1) -> -Integer.compare(ints[1], t1[1]));
        }

        public void update(int timestamp, int price) {
            current = Math.max(current, timestamp);
            int[] ints = new int[]{timestamp, price};
            hash.put(timestamp, ints);
            minQueue.add(ints);
            maxQueue.add(ints);
        }

        public int current() {
            return hash.get(current)[1];
        }
