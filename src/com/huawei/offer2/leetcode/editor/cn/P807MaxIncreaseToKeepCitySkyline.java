...
public class P807MaxIncreaseToKeepCitySkyline {
    public static void main(String[] args) {
        ...
        int[][] ints = ListNode.getIntArrays("[[3,0,8,4],[2,4,5,7],[9,2,6,3],[0,3,1,0]]");
        System.out.println(solution.maxIncreaseKeepingSkyline(ints));
    }
    ...
    class Solution {
        public int maxIncreaseKeepingSkyline(int[][] grid) {
            ...
            for (int i = 0; i < length; i++) {
                for (int j = 0; j < length; j++) {
                    sum += Math.min(ns[i], ms[j]) - grid[i][j];
                }
            }
            return sum;
        }
