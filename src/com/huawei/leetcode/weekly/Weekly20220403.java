    ...
    public List<List<Integer>> findWinners(int[][] matches) {
        ...
        for (int[] match : matches) {
            ...
            if (hashMap.containsKey(winner)) {
                int[] ints = hashMap.get(winner);
                ints[0] = ints[0] + 1;
            } else {
                int[] ints = new int[2];
                ints[0] = 1;
                hashMap.put(winner, ints);
            }
            if (hashMap.containsKey(loser)) {
                int[] ints = hashMap.get(loser);
                ints[1] = ints[1] + 1;
            } else {
                int[] ints = new int[2];
                ints[1] = 1;
                hashMap.put(loser, ints);
            }
        }
        List<Integer> collect =
                hashMap.keySet()
                        .stream()
                        .filter(integer -> hashMap.get(integer)[0] == 0)
                        .sorted()
                        .collect(Collectors.toList());
        List<Integer> collect2 =
                hashMap.keySet()
                        .stream()
                        .filter(integer -> hashMap.get(integer)[1] == 1)
                        .sorted()
                        .collect(Collectors.toList());
