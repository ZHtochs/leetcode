...
*/
    ...
    class Solution {
        public Node cloneGraph(Node node) {
            ...
            nodeMap.put(node, copyRoot);
            buildMap(nodeMap, node);
            for (Node node1 : nodeMap.keySet()) {
                for (Node neighbor : node1.neighbors) {
                    nodeMap.get(node1).neighbors.add(nodeMap.get(neighbor));
                }
            }
            return nodeMap.get(node);
        }

        private void buildMap(Map<Node, Node> nodeMap, Node node) {
            for (Node neighbor : node.neighbors) {
                if (!nodeMap.containsKey(neighbor)) {
                    Node node1 = new Node(neighbor.val);
                    nodeMap.put(neighbor, node1);
                    buildMap(nodeMap, neighbor);
                }
            }
        }
    }
